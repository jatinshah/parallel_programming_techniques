...
void *beginminSP(void *arg){
	...
	// Take two minSPs on different chromosomes
		...
		// Place where translocation is performed
		printf("Genome 1 before bad translocation at %d and %d by thread %d\n",g1->name,g2->name,(int)arg);
		print_genome(genome1_head);
		minSPtranslocation(g1,g2);
		printf("Genome 1 after bad translocation at %d and %d by thread %d\n",g1->name,g2->name,(int)arg);
		print_genome(genome1_head);
		pthread_mutex_lock(&pcycle_head_lock);
	}
	pthread_mutex_unlock(&pcycle_head_lock);
